    ...
    if user_id != CEO:
        ...
        try:
            ...
            if pmresults == 'None':
                ...
                value = "PREMIUM"
                updatedata(pmid, module_name, value)
                ad_resp = f"""USER <a href="tg://user?id={pmid}">{pmid}</a> SUCCCESSFULLY PURCHSEF STARTER PLAN AT 0.99$"""
                await message.reply_text(pmid, ad_resp)
                receipt_id = random.random()
                user_resp = f"""
...
Have Good Day - @K3VIN_X
        """
                await Client.send_message(user_resp, message.id)
