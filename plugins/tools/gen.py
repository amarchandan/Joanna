    ...
    try:
        ...
        if results == "None":
            ...
            if (
                ...
                if len(bin) == 0:
                    nocc = """
 Invalid input. Please provide a BIN /gen 562236xxxx |xx|xxx|xxx 
          """
...
Generating card....
              """
                ...
                Type = x.get("Type", "N/A")
                quantity = 15
                cc_generator = await GeneatedCC(bin)
                ccc = [
                    f"<code>{cc}</code>"
                    for cc in cc_generator.lista_tarjetas[:quantity]
                ]
                xxx = "\n".join(ccc)
                resp = f"""
Bin - <code>{bin}</code>
...
"""
                await Client.edit_message_text(message.chat.id, firstchk.id, resp)
